...
class BaseEnvironment(gym.Env):
    ...
    def __init__(self, config: dict) -> None:
        ...
        self.map_indices = np.arange(2, 2 + 32 * 100, 32) if evaluation_mode else np.arange(1, 1 + 16 * 1000, 16)
    ...
    def step(
        ...
        r = self.calc_capacity([(row, col)])  # capacity reward
        ...
        if not os.path.exists(filename):
            ...
            for actions in all_actions:
                ...
                if self.reward_type == 'coverage':
                    ...
                    reward = self.calc_capacity(tx_locs)
    ...
    def calc_capacity(self, tx_locs: list[tuple]) -> float:
        """Calculate the capacity reward of a TX, given its location.

        Args:
            tx_locs: [(row_1, col_1), (row_2, col_2) ...].

        Returns:
            Sum of normalized grayscale value of RoI pixels.

        """
        row, col = tx_locs[0][0], tx_locs[0][1]
        loc_idx = row * self.map_size + col
        pmap_filename = f'pmap_{self.map_idx}_{loc_idx}.png'
        pmap_dir = os.path.join(self.dataset_dir, 'power_map', f'{self.map_idx}')
        pmap_path = os.path.join(pmap_dir, pmap_filename)
        ...
        avg_capacity = power_map.sum() / num_roi
        return float(avg_capacity)

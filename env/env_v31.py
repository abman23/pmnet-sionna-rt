...
class BaseEnvironment(gym.Env):
    """MDP environment of PMNet-based multi-BS, where agents take actions asynchronously (use old dataset).
    ...
    def __init__(self, config: dict) -> None:
        ...
        if not evaluation_mode:
            self.map_indices = np.arange(1, 1 + 16 * 800, 16)
        else:
            self.map_indices = np.arange(1 + 16 * 800, 1 + 16 * 1000, 16)
